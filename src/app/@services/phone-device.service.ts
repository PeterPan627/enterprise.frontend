...
export class PhoneDeviceService {
  ...
  constructor(
    ...
    if (this.hasCommunicationsService()) {
      this.communicationsService.getAccessToken().subscribe(
        (r) => {
          try {
  ...
  private hasCommunicationsService(): boolean {
    const feature = this.configurationSevice.configuration.tenant.features.find(
      (f) => f.name === this.featureName
    );
    return !(feature === null || feature === undefined);
  }

  private showIncomingCall(
    caller: string,
    onAccept?: () => void,
    onReject?: () => void
  ) {
    const incomingCall: NgElement & WithProperties<IncomingCallComponent> =
      document.createElement("ngx-incoming-call") as any;
  ...
  private showConnectedCall(
    participant: string,
    onDial?: (num) => void,
    onHangup?: () => void
  ) {
    const connectedCall: NgElement & WithProperties<ConnectedCallComponent> =
      document.createElement("ngx-connected-call") as any;
